...
import { generateColabNotebookCode, generateJupyterNotebookCode } from '../services/geminiService';
...
const ReportDisplay: React.FC<ReportDisplayProps> = ({ report, onReset, wasSampleAnalyzed, onDownloadSample, rawData }) => {
  ...
  const [isGeneratingColabNotebook, setIsGeneratingColabNotebook] = useState(false);
  const [isGeneratingJupyterNotebook, setIsGeneratingJupyterNotebook] = useState(false);
  ...
  const createNotebookContent = (genericSummary: string, pythonCode: string, notebookType: 'colab' | 'jupyter') => {
    const cells = [];
    const header = [
        `# ${report.title}\n\n`,
    ...
    if (notebookType === 'colab') {
        header.push(`**Instructions:** This notebook is designed to work with data that has the same columns as the original file. Run each code cell sequentially, starting with the file upload prompt, to generate key metrics and visualizations for your own data.`);
        cells.push({
            cell_type: 'markdown',
            metadata: {},
            source: header
    ...
    // Split by the cell separator, but keep the separator with the following block.
    // The multiline flag 'm' is crucial for '^' to match start of line.
    const blocks = pythonCode.trim().split(/(?=^\s*#\s?%%\s*)/m);

    let isFirstBlock = true;
    let jupyterHeaderAdded = false;

    for (const block of blocks) {
        if (!block.trim()) continue;

        const separatorMatch = block.match(/^\s*#\s?%%\s*(\[markdown\])?\s*/);

        if (separatorMatch) {
            // Block starts with a separator, it's a new cell.
            const isMarkdown = separatorMatch[1] === '[markdown]';
            let content = block.substring(separatorMatch[0].length).trim();
            
            if (isMarkdown) {
                if (notebookType === 'jupyter' && !jupyterHeaderAdded) {
                    content = [...header, content].join('\n');
                    jupyterHeaderAdded = true;
                }
                cells.push({ cell_type: 'markdown', metadata: {}, source: content });
            } else {
                cells.push({ cell_type: 'code', execution_count: null, metadata: {}, outputs: [], source: content });
            }
        } else if (isFirstBlock) {
            // Content before the first separator. Assume it's code (for Colab notebooks).
            cells.push({ cell_type: 'code', execution_count: null, metadata: {}, outputs: [], source: block.trim() });
        }
        isFirstBlock = false;
    }
  ...
  const handleGenerateColabNotebook = async () => {
    setIsGeneratingColabNotebook(true);
    try {
      const { genericSummary, pythonCode } = await generateColabNotebookCode(report);
      const notebookContent = createNotebookContent(genericSummary, pythonCode, 'colab');

      const blob = new Blob([notebookContent], { type: 'application/vnd.jupyter.notebook+json' });
      const link = document.createElement('a');
      const url = URL.createObjectURL(blob);
      link.href = url;
      link.setAttribute('download', `Data-Insight-Colab-Notebook-${report.title.replace(/\s/g, '-')}.ipynb`);
      ...
      alert("An error occurred while generating the Google Colab Notebook. The AI model might be busy. Please try again.");
    } finally {
      setIsGeneratingColabNotebook(false);
    }
  };

  const handleGenerateJupyterNotebook = async () => {
    setIsGeneratingJupyterNotebook(true);
    try {
      const { genericSummary, pythonCode } = await generateJupyterNotebookCode(report);
      const notebookContent = createNotebookContent(genericSummary, pythonCode, 'jupyter');

      const blob = new Blob([notebookContent], { type: 'application/vnd.jupyter.notebook+json' });
      const link = document.createElement('a');
      const url = URL.createObjectURL(blob);
      link.href = url;
      link.setAttribute('download', `Data-Insight-Jupyter-Notebook-${report.title.replace(/\s/g, '-')}.ipynb`);
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
    } catch (error) {
      console.error("Error generating Jupyter notebook:", error);
      alert("An error occurred while generating the Jupyter Notebook. The AI model might be busy. Please try again.");
    } finally {
      setIsGeneratingJupyterNotebook(false);
    }
  };
    ...
    <>
      ...
      <div className="text-center pt-8 mt-12 border-t border-slate-200">
        <div className="flex flex-wrap justify-center items-center gap-4">
            ...
            <button
                onClick={handleGenerateColabNotebook}
                disabled={isGeneratingColabNotebook}
                className="w-full sm:w-auto inline-flex justify-center items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-blue-400 disabled:cursor-not-allowed transition-all"
                aria-live="polite"
            >
                {isGeneratingColabNotebook ? (
                    <>
                        <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
             ...
             <button
                onClick={handleGenerateJupyterNotebook}
                disabled={isGeneratingJupyterNotebook}
